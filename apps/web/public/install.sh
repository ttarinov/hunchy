...
RELEASES_URL="https://hunchy-4a0dc.web.app"
  ...
  case "$user_shell" in
    ...
    */bash)
      ...
      if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        # Also add to .bashrc if it exists (some users source it from .bash_profile)
...
main() {
  ...
  echo ""
  warn "Please restart your terminal to use the 'hunchy' command"
  echo ""
  echo "  Note: The first time you run 'hunchy', it will download"
  echo "  the CLI runtime (~28MB). This only happens once."
  echo ""
  echo "  Get started:"
  echo "    hunchy --help   # Show available commands"
